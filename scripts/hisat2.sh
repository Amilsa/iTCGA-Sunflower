...
for file in "$input_dir"/*_trimmed_R1_001_downsampled.fastq; do
  # Extract the base name without the suffix (e.g., "C1_S4_L001")
  base=$(basename "$file" _trimmed_R1_001_downsampled.fastq)

  hisat2 -p 24 \
  -x "$index_dir" \
  -1 "$input_dir/${base}_trimmed_R1_001_downsampled.fastq" \
  -2 "$input_dir/${base}_trimmed_R2_001_downsampled.fastq" \
